#!/usr/bin/env python3
"""Lint a Doubao pure-text audio prompt for the Cat Shopkeeper workflow."""
...
ROLE_RE = re.compile(r"^.{1,120}(?:是|为).{2,}(?:男|女|声线|音色|口音|普通话|方言).*$")
SPEECH_RE = re.compile(r"^.{1,160}(?:说道|说：|说话|回应|解释|喊道|问道|低声|大声|怒吼|回击).*[“\"].+[”\"]")
QUOTE_RE = re.compile(r"[“\"].+?[”\"]")
SOUND_RE = re.compile(r"音乐|环境|音效|声音|脚步|电话|震动|呼吸|碰撞|雨声|风声|鼓点|贝斯|合成器|钢琴|铜管|静音")


def lint(prompt: str, require_hk: bool) -> dict[str, object]:
    text = prompt.strip()
    lines = [line.strip() for line in text.splitlines() if line.strip()]
    errors: list[str] = []
    warnings: list[str] = []
    if len(text) < 120:
        errors.append("提示词过短，缺少官方模板所需的角色、声音环境和对白结构。")
    ...
    if not re.search(r"停顿|语速|音量|声线|尾音|情绪|咬字|低声|高声|加快|放慢", text):
        errors.append("没有检测到具体表演方向，不能只写抽象的‘自然’。")
    for phrase in GENERIC_ONLY:
        if phrase in text:
            warnings.append(f"发现泛化句式“{phrase}”，请改成角色、速度、音量、停顿和情绪变化。")
    if re.search(r"模仿|像.*周星驰|像.*刘德华|像.*梁朝伟|复制.*声音|电影原声", text):
        errors.append("不能要求复制具体演员或电影原声；请改写为年代、口音和表演风格。")
    if len(QUOTE_RE.findall(text)) < len(speech_lines):
        warnings.append("部分对白可能没有使用成对中文引号，请人工检查。")
    return {"ok": not errors, "errors": errors, "warnings": warnings, "stats": {"characters": len(text), "role_lines": len(role_lines), "speech_lines": len(speech_lines), "quoted_lines": len(QUOTE_RE.findall(text))}}
...
阿杰先惊讶吸气，随后放慢语速回应：“这双鞋原来还能这么干净？”结尾保留一秒雨声，不再加对白。"""
    ...
    assert lint(good, require_hk=False)["ok"], lint(good, require_hk=False)
    assert not lint(bad, require_hk=False)["ok"], lint(bad, require_hk=False)
    assert not lint(good, require_hk=True)["ok"], lint(good, require_hk=True)
...
def main() -> int:
    ...
    require_hk = args.style == "hk90" or (args.style == "auto" and re.search(r"90年代|九十年代|香港电影|港片|港式", text) is not None)
    result = lint(text, require_hk=require_hk)
