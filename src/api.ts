...
export const api = {
  ...
  updateAudioAsset: (audioId: string, body: { remoteUrl: string; duration?: number; rightsNote: string; description: string }) => request(`/api/audio-assets/${audioId}`, { method: "PUT", body: JSON.stringify(body) }),
  audioVoiceLockImpact: (audioId: string) => request<{ replacingAudioId: string | null; affectedShots: Array<{ id: string; shotNumber: number; title: string }>; activeVideoJobs: unknown[] }>(`/api/audio-assets/${audioId}/lock-impact`),
  lockVoiceAsset: (audioId: string, confirmInvalidation = false) => request(`/api/audio-assets/${audioId}/lock-voice`, { method: "POST", body: JSON.stringify({ confirmInvalidation }) }),
  registerSeedanceVoice: (audioId: string) => request(`/api/audio-assets/${audioId}/register-seedance`, { method: "POST", headers: { "x-workbench-paid-confirm": "register-seedance-voice" } }),
  ...
  requestShotImageRevision: (shotId: string, body: { feedback: string }) => request(`/api/shots/${shotId}/image-revision`, { method: "POST", body: JSON.stringify(body) }),
