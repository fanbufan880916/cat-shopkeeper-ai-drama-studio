...
import { audioPromptContextFromScript, audioStyleLabels, buildAudioPrompt, buildVoiceAnchorPrompt, inferAudioStyleProfile, validateAudioPrompt, validateVoiceAnchorPrompt } from "../shared/audio-prompt";
...
import { lockedVoiceForCharacter, matchCharacterAsset } from "../shared/voice-anchors";
...
function Studio() {
  ...
  const speakingRoles = [...new Set(extractLockedScriptDialogue(data.artifacts, data.shots).map((line) => line.speaker))];
  const readyVoiceRoles = speakingRoles.filter((speaker) => {
    const character = matchCharacterAsset(speaker, data.assets);
    return Boolean(character && lockedVoiceForCharacter(data.audioAssets, character.id)?.seedanceAssetUrl.startsWith("asset://"));
  });
  const audioComplete = speakingRoles.length === 0 || readyVoiceRoles.length === speakingRoles.length;
  return (
    <div className="studio-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="项目制作导航">{navigation.map(([section, path, label, Icon]) => {
          const state = getNavigationState(data.project.stage, section, hasScript, { audio: audioComplete });
...
function StoryboardPage({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  ...
  const [imageReviewingId, setImageReviewingId] = useState("");
  ...
  function compiledVideoPrompt(shot: DashboardData["shots"][number]) {
    ...
    const refs = shot.audioMode === "voice_reference"
      ? shot.voiceBindings.map((binding, index) => ({ asset: lockedVoiceForCharacter(data.audioAssets, binding.characterAssetId), speaker: binding.speaker, tag: `@Audio${index + 1}` })).filter((item) => item.asset)
      : shot.audioAssetIds.map((audioId, index) => ({ asset: data.audioAssets.find((item) => item.id === audioId), speaker: "", tag: `@Audio${index + 1}` })).filter((item) => item.asset);
    const referenceMap = refs.map(({ asset, tag, speaker }) => `${tag} ${asset!.type === "character_voice" ? `只提供${speaker || "该角色"}的年龄感、音色、口音和说话质感，不复用示例台词` : asset!.type === "music" ? "控制音乐节拍、段落和表演时钟" : asset!.type === "ambience" ? "控制环境底噪" : "控制关键音效时点"}`).join("；");
  ...
  async function requestShotImageRevision(shot: DashboardData["shots"][number]) {
    const feedback = imageFeedback[shot.id]?.trim() ?? "";
    if (!feedback) { setMessage("退回分镜首帧前，请填写具体修改意见。"); return; }
    setImageReviewingId(shot.id); setMessage("");
    try {
      await api.requestShotImageRevision(shot.id, { feedback });
      setImageFeedback((current) => ({ ...current, [shot.id]: "" }));
      setMessage(`镜头 ${shot.shotNumber} 首帧已退回。旧图片仍保留，系统不会自动改提示词或重新生图。`);
      await reload();
    } catch (error) { setMessage((error as Error).message); } finally { setImageReviewingId(""); }
  }
  ...
  return <>
    ...
    <div className="shot-list">{data.shots.map((shot) => {
      ...
      const shotAudioAssets = shot.audioMode === "voice_reference"
        ? shot.voiceBindings.map((binding) => lockedVoiceForCharacter(data.audioAssets, binding.characterAssetId)).filter(Boolean)
        : shot.audioAssetIds.map((id) => data.audioAssets.find((item) => item.id === id)).filter(Boolean);
      const uniqueVoiceCharacterIds = new Set(shot.voiceBindings.map((binding) => binding.characterAssetId).filter(Boolean));
      const voiceAudioProblem = shot.audioMode !== "voice_reference" || !shot.dialogue.trim() ? ""
        : shot.voiceBindings.length === 0 ? "当前对白没有说话人角色绑定。"
        : shot.voiceBindings.length > 3 ? "当前镜头超过3个说话角色，请先拆分镜头。"
        : shot.voiceBindings.some((binding) => !binding.characterAssetId) ? "有说话人尚未绑定角色资产。"
        : shotAudioAssets.length < uniqueVoiceCharacterIds.size ? "有说话角色尚未锁定音色。"
        : shotAudioAssets.some((item) => !item!.seedanceAssetUrl.startsWith("asset://")) ? "有角色音色尚未通过Seedance审核。"
        : shotAudioAssets.reduce((sum, item) => sum + item!.duration, 0) > 15 ? "角色音色参考总时长超过15秒，请缩短样本或拆分镜头。"
        : "";
      const missingRemoteAudio = shot.audioMode === "voice_reference" ? Boolean(voiceAudioProblem) : shotAudioAssets.some((item) => !item!.remoteUrl);
      ...
      const openImageRevision = data.revisions.find((revision) => revision.targetType === "image" && revision.targetId === shot.id && revision.status !== "resolved");
      const imageStatus = openImageRevision ? "已退回修改" : shot.approvedImageMediaId ? "首帧已锁定" : jobs.some((job) => data.mediaFiles.some((media) => media.jobId === job.id && media.kind === "image")) ? "等待选择首帧" : "等待生成";
      return <article id={`shot-${shot.id}`} className="shot-card shot-card-generation" key={shot.id}>
        ...
        <div className="shot-main">
          ...
          <div className="shot-generation-panel">
            ...
            {jobs.some((job) => job.status !== "failed") && <div className="shot-image-results">{jobs.filter((job) => job.status !== "failed").flatMap((job) => { const media = data.mediaFiles.filter((item) => item.jobId === job.id && item.kind === "image"); if (!media.length) return [<div className="shot-result-placeholder" key={job.id}><span>{job.progress}%</span><small>{job.provider === "codex" ? "Codex" : job.model}</small></div>]; return media.map((item, index) => <article className={clsx("shot-image-candidate", shot.approvedImageMediaId === item.id && "locked")} key={item.id}><button type="button" className="shot-image-preview" onClick={() => setLightboxUrl(`/api/media/${item.id}`)}><img src={`/api/media/${item.id}`} alt={`镜头${shot.shotNumber}首帧候选${index + 1}`} /><small>{job.provider === "codex" ? "Codex" : job.model} · {index + 1}</small></button><button className={shot.approvedImageMediaId === item.id ? "ghost" : "primary"} disabled={lockingImageId === item.id} onClick={() => lockShotImage(shot.id, job.id, item.id)}>{shot.approvedImageMediaId === item.id ? "已锁定首帧" : lockingImageId === item.id ? "正在锁定…" : "锁定此图"}</button></article>); })}</div>}
            <section className="shot-image-review"><div><strong>分镜首帧确认</strong><span>{imageStatus}</span></div>{shot.approvedImageMediaId ? <p className="asset-lock-success"><LockKeyhole size={14} />退回后会清除当前首帧锁定及该镜头视频状态；旧图片和任务仍保留。</p> : <p className="asset-review-warning">返工只针对当前镜头首帧，不等于退回完整分镜，也不会自动生图。</p>}<label>返工意见<textarea value={imageFeedback[shot.id] ?? ""} onChange={(event) => setImageFeedback((current) => ({ ...current, [shot.id]: event.target.value }))} placeholder="例如：人物脸型不一致、服装错误、手部变形、人物站位错误、构图太挤、场景与参考图不一致。" /></label><button className="danger" disabled={imageReviewingId === shot.id || !(imageFeedback[shot.id]?.trim())} onClick={() => void requestShotImageRevision(shot)}>{imageReviewingId === shot.id ? "正在退回…" : "退回修改"}</button>{openImageRevision && <small>当前开放意见：{openImageRevision.feedback}</small>}</section>
          ...
          <div className="shot-video-panel">
            ...
            <div className="multimodal-reference-panel"><div className="production-module-heading"><div><strong>本镜头参考资产</strong><span>最多 9 张图片、3 段音频、3 段视频；对白音色自动按台词首次出现顺序解析</span></div><span className="reference-count">{shot.assetIds.length + (shot.approvedImageMediaId ? 1 : 0)} 图 · {shotAudioAssets.length} 音频 · {shot.videoReferenceMediaIds.length} 视频</span></div><div className="reference-asset-grid">{linkedAssets.filter((asset) => asset.referenceMediaId).map((asset, index) => <button type="button" className="reference-asset-card" key={asset.id} onClick={() => setLightboxUrl(`/api/assets/${asset.id}/reference`)}><img loading="lazy" width="72" height="96" src={`/api/assets/${asset.id}/reference`} alt={asset.name} /><b>@Image{index + 1}</b><span>{asset.name}</span></button>)}{shotAudioAssets.map((audio, index) => <div className="reference-asset-card audio-reference-card" key={audio!.id}><b>@Audio{index + 1}</b><span>{audio!.name}</span><small>{audio!.type === "character_voice" ? audio!.seedanceAssetUrl ? "asset:// 已就绪" : "待Seedance审核" : audio!.remoteUrl ? "远程地址已就绪" : "缺少远程地址"}</small></div>)}{data.mediaFiles.filter((media) => media.kind === "video" && Boolean(media.sourceUrl)).map((media) => <label className={clsx("reference-asset-card", shot.videoReferenceMediaIds.includes(media.id) && "selected")} key={media.id}><input type="checkbox" checked={shot.videoReferenceMediaIds.includes(media.id)} disabled={!shot.videoReferenceMediaIds.includes(media.id) && shot.videoReferenceMediaIds.length >= 3} onChange={() => void updateShotVideoReferences(shot, media.id)} /><b>@Video{shot.videoReferenceMediaIds.indexOf(media.id) + 1 || "·"}</b><span>已完成视频参考</span><small>HTTPS 已就绪</small></label>)}</div>{(shotAudioAssets.length > 0 || shot.videoReferenceMediaIds.length > 0) && <p className="compatibility-note">有音频或视频参考时，系统会自动使用 image_urls 多模态模式，不会同时提交 image_with_roles。</p>}</div>
...
function JobsPage({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  ...
  const jobs = sortTasks(data.jobs.filter((job) => {
    const object = taskObject(job, data.assets, data.shots, data.audioAssets);
...
function AudioProductionPageWithLibrary({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  return <>
    <VoiceAnchorCenter data={data} reload={reload} />
    <details className="panel audio-history-panel">
      <summary>历史 / 精确台词素材</summary>
      <p>旧场景母带和台词切片继续保留用于试听与查档，但不再作为声音完成条件，也不会提交给Seedance。</p>
      <AudioLibraryPreview data={data} reload={reload} />
    </details>
  </>;
}

function VoiceAnchorCenter({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  const navigate = useNavigate();
  const lockedScript = useMemo(() => latestLockedScript(data.artifacts), [data.artifacts]);
  const scriptLines = useMemo(() => extractLockedScriptDialogue(data.artifacts, data.shots), [data.artifacts, data.shots]);
  const context = useMemo(() => audioPromptContextFromScript(lockedScript?.content ?? ""), [lockedScript]);
  const roles = useMemo(() => [...new Set(scriptLines.map((line) => line.speaker))].map((speaker) => {
    const lines = scriptLines.filter((line) => line.speaker === speaker);
    const character = matchCharacterAsset(speaker, data.assets);
    const profile = context.characters.find((item) => item.name.split(/[/／、]/).map((name) => name.trim()).includes(speaker));
    return { speaker, lines, character, profile };
  }), [context.characters, data.assets, scriptLines]);
  const [promptDrafts, setPromptDrafts] = useState<Record<string, string>>({});
  const [remoteDrafts, setRemoteDrafts] = useState<Record<string, { remoteUrl: string; duration: string }>>({});
  const [busyId, setBusyId] = useState("");
  const [message, setMessage] = useState("");
  useEffect(() => {
    if (!lockedScript) return;
    setPromptDrafts((current) => Object.fromEntries(roles.map((role) => {
      const sourceLine = role.lines[0];
      const shortText = sourceLine?.text.length > 22 ? `${sourceLine.text.slice(0, 21)}。` : sourceLine?.text ?? "请确认我的声音。";
      const prompt = buildVoiceAnchorPrompt({ speaker: role.speaker, text: shortText, voiceDirection: sourceLine?.voiceDirection || role.profile?.voiceDirection }, { script: lockedScript.content });
      return [role.speaker, current[role.speaker] ?? prompt];
    })));
  }, [lockedScript, roles]);

  const roleState = roles.map((role) => {
    const candidates = role.character ? data.audioAssets.filter((audio) => audio.type === "character_voice" && audio.characterAssetId === role.character!.id).sort((a, b) => b.version - a.version) : [];
    const locked = candidates.find((audio) => audio.status === "locked") ?? null;
    return { ...role, candidates, locked, ready: Boolean(locked?.seedanceAssetUrl.startsWith("asset://")) };
  });
  const lockedCount = roleState.filter((role) => role.locked).length;
  const readyCount = roleState.filter((role) => role.ready).length;

  async function generate(role: (typeof roleState)[number]) {
    if (busyId || !role.character) return;
    if (data.project.dryRun) { setMessage("空跑测试项目不会调用真实豆包音频接口。"); return; }
    const textPrompt = promptDrafts[role.speaker] ?? "";
    const errors = validateVoiceAnchorPrompt(textPrompt, context.style);
    if (errors.length) { setMessage(`“${role.speaker}”提示词未通过检查：${errors.join("；")}`); return; }
    setBusyId(`generate-${role.speaker}`); setMessage("");
    try {
      await api.generateAudioAsset(data.project.id, {
        type: "character_voice", name: `${role.speaker}·音色候选V${role.candidates.length + 1}`, characterAssetId: role.character.id,
        textPrompt, styleProfile: context.style, speaker: "", referenceAudioUrls: [], format: "wav", sampleRate: 24000,
        enableSubtitle: false, speechRate: 0, pitchRate: 0, loudnessRate: 0,
        rightsNote: "本人拥有或已获授权使用该声音；仅用于本项目", description: `${role.speaker}的4到5秒单角色干声音色锚点。`,
        contentProducer: "猫掌柜AI漫剧工作台", contentPropagator: "猫掌柜工作室", aigcWatermark: true, enableWatermark: true
      });
      setMessage(`“${role.speaker}”的新音色候选已提交；完成后请试听，再手动锁定。`);
      await reload();
    } catch (error) { setMessage((error as Error).message); } finally { setBusyId(""); }
  }
  async function saveSource(audio: DashboardData["audioAssets"][number]) {
    const draft = remoteDrafts[audio.id] ?? { remoteUrl: audio.remoteUrl, duration: String(audio.duration || "") };
    setBusyId(`source-${audio.id}`); setMessage("");
    try {
      await api.updateAudioAsset(audio.id, { remoteUrl: draft.remoteUrl.trim(), duration: Number(draft.duration || 0), rightsNote: audio.rightsNote, description: audio.description });
      setMessage("Seedance审核源地址和时长已保存。"); await reload();
    } catch (error) { setMessage((error as Error).message); } finally { setBusyId(""); }
  }

  async function lock(audio: DashboardData["audioAssets"][number]) {
    setBusyId(`lock-${audio.id}`); setMessage("");
    try {
      const impact = await api.audioVoiceLockImpact(audio.id);
      if (impact.activeVideoJobs.length) throw new Error("受影响镜头仍有视频任务处理中，暂时不能更换音色。");
      const confirmInvalidation = Boolean(impact.replacingAudioId && impact.affectedShots.length);
      if (confirmInvalidation && !window.confirm(`确认更换角色音色？\n\n将失效 ${impact.affectedShots.length} 个镜头：${impact.affectedShots.map((shot) => `镜头${shot.shotNumber}·${shot.title}`).join("、")}\n已提交的历史任务不会被改写。`)) return;
      await api.lockVoiceAsset(audio.id, confirmInvalidation);
      setMessage("角色音色已锁定。下一步请手动提交Seedance音色审核。"); await reload();
    } catch (error) { setMessage((error as Error).message); } finally { setBusyId(""); }
  }

  async function register(audio: DashboardData["audioAssets"][number]) {
    if (!window.confirm(`确认把“${audio.name}”提交给Seedance进行私有音色审核？\n\n这会调用APIMart真实接口，可能产生费用。`)) return;
    setBusyId(`register-${audio.id}`); setMessage("");
    try { await api.registerSeedanceVoice(audio.id); setMessage("Seedance音色审核已提交。工作台会自动轮询，成功后显示 asset:// 已就绪。"); await reload(); }
    catch (error) { setMessage((error as Error).message); } finally { setBusyId(""); }
  }

  if (!lockedScript) return <><PageHeader eyebrow="VOICE ANCHORS" title="角色音色锚点中心" description="声音生产只读取用户已经通过的锁定剧本。" /><section className="panel prerequisite-panel"><Volume2 size={28} /><div><h2>当前没有已锁定剧本</h2><p>请先完成剧本审核，避免基于仍在修改的角色信息生成音色。</p></div><button className="primary" onClick={() => navigate(projectRoute(data.project.id, "script"))}>返回剧本审片</button></section></>;
  if (!roles.length) return <><PageHeader eyebrow="VOICE ANCHORS" title="角色音色锚点中心" description="当前锁定剧本没有可识别对白。" /><section className="panel audio-confirmation-summary complete"><span className="audio-confirmation-check"><Check size={20} /></span><div><strong>无需角色音色</strong><p>当前项目没有说话角色，声音导航已自动完成。</p></div><b>0/0</b></section></>;
  return <>
    <PageHeader eyebrow="VOICE ANCHORS" title="角色音色锚点中心" description="每个说话角色只锁定一条4到5秒干声音色；Seedance只参考音色，目标台词仍在镜头提示词中重新写明。" />
    <section className={clsx("panel audio-confirmation-summary", readyCount === roles.length && "complete")} aria-live="polite"><span className="audio-confirmation-check"><Check size={20} /></span><div><strong>{readyCount === roles.length ? "全部角色音色已就绪" : "角色音色等待确认"}</strong><p>说话角色 {roles.length} 个 · 已锁定 {lockedCount} 个 · Seedance已就绪 {readyCount} 个</p></div><b>{readyCount}/{roles.length}</b></section>
    {context.style === "needs_review" && <section className="alert error-text" role="alert">锁定剧本没有明确时代、地域、语言、口音和表演方式，当前不能提交真实音色生成。</section>}
    <section className="voice-anchor-list">{roleState.map((role) => <article className="panel voice-anchor-card" key={role.speaker}>
      <header><div><span>说话角色</span><h2>{role.speaker}</h2></div><Status value={role.ready ? "completed" : role.locked ? "processing" : "draft"} /></header>
      <div className="voice-anchor-meta"><span><b>角色资产</b>{role.character?.name ?? "未唯一匹配"}</span><span><b>剧本声线要求</b>{role.profile?.voiceDirection || role.lines[0]?.voiceDirection || audioStyleLabels[context.style]}</span></div>
      {!role.character && <p className="error-text" role="alert">找不到唯一角色资产。请先让分镜Agent把“{role.speaker}”与正确角色资产绑定。</p>}
      <label>音色锚点提示词<textarea value={promptDrafts[role.speaker] ?? ""} onChange={(event) => setPromptDrafts((current) => ({ ...current, [role.speaker]: event.target.value }))} /></label>
      <button className="primary" disabled={!role.character || context.style === "needs_review" || data.project.dryRun || Boolean(busyId)} onClick={() => void generate(role)}><Sparkles size={16} />{busyId === `generate-${role.speaker}` ? "正在提交…" : "生成新候选"}</button>
      <div className="voice-candidate-list">{role.candidates.map((audio) => {
        const job = data.jobs.find((item) => item.audioAssetId === audio.id && item.kind === "audio");
        const registration = audio.registrationJobId ? data.jobs.find((item) => item.id === audio.registrationJobId) : undefined;
        const draft = remoteDrafts[audio.id] ?? { remoteUrl: audio.remoteUrl, duration: String(audio.duration || "") };
        return <section className={clsx("voice-candidate", audio.status === "locked" && "locked")} key={audio.id}><div className="voice-candidate-heading"><strong>V{audio.version} · {audio.name}</strong><span>{audio.seedanceAssetUrl ? "Seedance已就绪" : audio.status === "locked" ? "已锁定，待Seedance审核" : job?.status === "failed" ? "生成失败" : job && ["draft", "submitted", "processing"].includes(job.status) ? "生成中" : "待试听"}</span></div>
          {audio.localPath || /^https:\/\//i.test(audio.remoteUrl) ? <audio controls preload="metadata" src={audio.localPath ? `/api/audio-assets/${audio.id}/file` : audio.remoteUrl} /> : <small>{job?.status === "failed" ? job.error : "试听文件尚未就绪"}</small>}
          <div className="voice-source-fields"><label>Seedance可访问的HTTPS样本<input value={draft.remoteUrl} onChange={(event) => setRemoteDrafts((current) => ({ ...current, [audio.id]: { ...draft, remoteUrl: event.target.value } }))} placeholder="只有本地/Base64时请填写有权使用的HTTPS地址" /></label><label>真实时长（秒）<input type="number" min="4" max="5.5" step="0.1" value={draft.duration} onChange={(event) => setRemoteDrafts((current) => ({ ...current, [audio.id]: { ...draft, duration: event.target.value } }))} /></label><button className="ghost" disabled={audio.status === "locked" || Boolean(busyId)} onClick={() => void saveSource(audio)}>保存审核源</button></div>
          <div className="button-row"><button className="ghost" disabled={(!audio.localPath && !/^https:\/\//i.test(audio.remoteUrl)) || audio.status === "locked" || Boolean(busyId)} onClick={() => void lock(audio)}><LockKeyhole size={15} />{audio.status === "locked" ? "当前角色音色" : "锁定为角色音色"}</button>{audio.status === "locked" && !audio.seedanceAssetUrl && <button className="primary" disabled={!audio.remoteUrl || Boolean(busyId) || ["draft", "submitted", "processing"].includes(registration?.status ?? "")} onClick={() => void register(audio)}>提交Seedance音色审核</button>}{audio.seedanceAssetUrl && <code>{audio.seedanceAssetUrl}</code>}</div>
          {registration?.status === "failed" && <p className="error-text" role="alert">Seedance审核失败：{registration.error}</p>}
        </section>;
      })}{!role.candidates.length && <Empty text="还没有音色候选。生成不会自动锁定，也不会自动提交Seedance。" />}</div>
    </article>)}</section>
    {message && <p className="notice wide" aria-live="polite">{message}</p>}
  </>;
}
...
function AudioLibraryPreview({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  ...
  const historicalAssets = data.audioAssets.filter((audio) => audio.type !== "character_voice");
  ...
  return <section className="panel audio-library-preview"><div className="section-heading"><div><p className="eyebrow">HISTORICAL AUDIO</p><h2>历史声音素材 · 仅试听查档</h2><p>旧母带、台词切片、音乐和音效保留在这里，不计入角色音色完成状态，也不会作为Seedance对白参考。</p></div><span>{historicalAssets.length} 项</span></div><div className="audio-preview-grid">{historicalAssets.map((audio) => <article key={audio.id}><div className="audio-preview-heading"><div><strong>{audio.name}</strong><span>{label(audio.type)}</span></div><button className="danger" type="button" disabled={Boolean(removingId)} onClick={() => void removeAudioAsset(audio)}>{removingId === audio.id ? "正在移除…" : "移除"}</button></div>{audio.localPath ? <audio controls preload="metadata" src={`/api/audio-assets/${audio.id}/file`} /> : <small className="audio-preview-pending">试听文件尚未就绪</small>}<small>{audio.duration ? `${audio.duration.toFixed(1)} 秒` : "等待时长"} · 历史素材</small></article>)}{!historicalAssets.length && <Empty text="暂无历史母带或台词素材。" />}</div>{message && <p className="notice" aria-live="polite">{message}</p>}</section>;
}
