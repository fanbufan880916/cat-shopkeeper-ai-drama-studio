    ...
    {selected && <div className="asset-modal-backdrop" onMouseDown={() => setSelectedId(null)}><section ref={assetModalRef} className="asset-modal" role="dialog" aria-modal="true" aria-labelledby="asset-modal-title" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className="asset-modal-grid">
        <div className="asset-generation-panel">
          ...
          {settings && !settings.hasApiKey && <p className="notice notice-error">APIMart Key 未配置，但仍可使用 Codex 额度通道。</p>}
...
function AudioProductionPage({ data, reload }: { data: DashboardData; reload: () => Promise<void> }) {
  const navigate = useNavigate();
  type DialogueDraft = { id: string; sceneId: string; sceneNumber: number; sceneName: string; shotId: string | null; shotNumber: number; title: string; speaker: string; text: string; description: string };
  ...
  const scriptContext = useMemo(() => {
    return lockedScript ? (typeof lockedScript.content === "string" ? lockedScript.content : JSON.stringify(lockedScript.content)) : "";
  }, [lockedScript]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <div className="button-row">
            <button className="primary" disabled={Boolean(busyId) || rows.length === 0} onClick={() => void generateMaster()}>
              <Sparkles size={17} />生成场景对白母带
            </button>
          </div>
        }
      />
      ...
      <section className="panel audio-master-panel">
        ...
        <div className="audio-master-prompt-actions">
          <button className="ghost" type="button" disabled={Boolean(busyId) || rows.length === 0} onClick={() => setMasterPrompt(buildAudioPrompt(rows, { style: audioStyle }))}>
            按已编辑台词重建官方模板
          </button>
        ...
        <div className="audio-master-actions">
          ...
          <button className="ghost" disabled={Boolean(busyId) || !selectedMaster?.localPath} onClick={() => void splitMaster()}>自动切成镜头片段</button>
        </div>
        {message && <p className="notice" aria-live="polite">{message}</p>}
      </section>
          ...
          return (
            <article className="panel audio-line-card" key={line.id}>
              <header>
                ...
                <Status value={clip?.status === "approved" ? "completed" : clip ? "processing" : "draft"} />
              </header>
