...
- 所有豆包音频生成、角色音色母版、场景对白母带和镜头音频切片，必须使用项目级 `$doubao-audio-generation` Skill。
- 先从锁定剧本判定时代、地域、语言、口音和表演风格，再按“角色声线清单 → 声音环境/音乐 → 按顺序对白与音效”的官方纯文本结构生成；只有剧本明确是90年代香港电影时才使用港式普通话。
- 禁止再使用“用稳定、清晰、自然的普通话说”作为完整提示词；必须写出角色声线、语速、音量、停顿和情绪变化。
- 生成前运行 `python .agents/skills/doubao-audio-generation/scripts/prompt_lint.py <prompt-file> --style auto`，失败不得提交真实任务；只有剧本明确是90年代港片时才使用 `--style hk90` 强制检查港式普通话。
